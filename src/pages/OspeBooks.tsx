...
export default function OspeBooks() {
  ...
  return (
    <div className="flex flex-col gap-6 max-w-5xl mx-auto">
      ...
      {/* PDF Preview Modal */}
      {activeBook && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setActiveBook(null)}>
          ...
          >
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <h3 style={{ fontFamily: FONT_DISPLAY, fontWeight: 700, fontSize: 16 }}>{activeBook.title}</h3>
                {activeBook.description && (
                  <p className="mt-1 text-xs" style={{ color: t.textMuted }}>{activeBook.description}</p>
                )}
              </div>
              <button
            ...
            {previewUrl ? (
              <div className="overflow-hidden rounded-xl" style={{ aspectRatio: "3 / 4", maxHeight: "70vh" }}>
                <iframe src={previewUrl} title={activeBook.title} className="h-full w-full" allow="autoplay" />
              </div>
            ) : (
              <div
                className="flex flex-col items-center justify-center gap-2 rounded-xl p-10 text-center"
                style={{ backgroundColor: t.surfaceAlt, border: `1.5px dashed ${t.border}` }}
              >
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="mt-3 flex w-fit items-center gap-1.5 text-xs font-bold hover:opacity-80"
              style={{ color: t.teal }}
            >
